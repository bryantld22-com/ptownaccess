...
export type EconomicsWorksheet = { id: string; artistId: string; eventDate: string; capacity: number; ticketPrice: number; expectedAttendance: number; foodSpendPerGuest: number; artistFee: number; productionCost: number; marketingCost: number; hospitalityCost: number; otherCost: number; updatedAt: string };
...
type OperationsState = { version: 1; artistUpdates: Record<string, ArtistUpdate>; bookings: BookingEntry[]; currentRole: OperationsRole; outreachDrafts: OutreachDraft[]; economics: EconomicsWorksheet[]; offers: OfferTerms[]; auditLog: AuditEntry[] };
type Store = OperationsState & { ready: boolean; busy: boolean; error: string | null; canEdit: boolean; canApprove: boolean; getArtist: (id: string) => ArtistLead | undefined; saveArtist: (id: string, update: ArtistUpdate) => Promise<boolean>; saveBooking: (booking: Omit<BookingEntry, 'id' | 'createdAt'>) => Promise<boolean>; removeBooking: (id: string) => Promise<boolean>; setRole: (role: OperationsRole) => Promise<boolean>; saveOutreachDraft: (draft: Omit<OutreachDraft, 'id' | 'createdAt' | 'updatedAt' | 'status'>) => Promise<boolean>; setDraftStatus: (id: string, status: OutreachDraft['status']) => Promise<boolean>; removeOutreachDraft: (id: string) => Promise<boolean>; saveEconomics: (worksheet: Omit<EconomicsWorksheet, 'id' | 'updatedAt'>) => Promise<boolean>; saveOffer: (offer: Omit<OfferTerms, 'id' | 'updatedAt' | 'status'>) => Promise<boolean>; setOfferStatus: (id: string, status: OfferTerms['status']) => Promise<boolean> };

const emptyState = (): OperationsState => ({ version: 1, artistUpdates: {}, bookings: [], currentRole: 'Owner', outreachDrafts: [], economics: [], offers: [], auditLog: [] });
...
export function parseOperationsState(raw: string | null): OperationsState {
  ...
  const economics = Array.isArray(value.economics) ? value.economics : [];
  const offers = Array.isArray(value.offers) ? value.offers : [];
  const auditLog = Array.isArray(value.auditLog) ? value.auditLog : [];
  return { version: 1, artistUpdates, bookings, currentRole, outreachDrafts, economics, offers, auditLog };
}
...
export function OperationsStoreProvider({ children }: PropsWithChildren) {
  ...
  return <Context.Provider value={{ ...state, ready, busy: pending > 0, error, canEdit, canApprove, getArtist, saveArtist, saveBooking, removeBooking, setRole, saveOutreachDraft, setDraftStatus, removeOutreachDraft, saveEconomics, saveOffer, setOfferStatus }}>{children}</Context.Provider>;
}
