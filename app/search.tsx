...
export default function Search() {
  ...
  const programs = events.filter(event => (filter === 'All' || filter === 'Programs' || (filter === 'Saved' && ready && savedEventIds.includes(event.id))) && matches([event.title, event.day, event.category, event.admission, event.description, event.opening, event.afterParty, event.tournamentFeature?.title, event.tournamentFeature?.description].join(' ')));
