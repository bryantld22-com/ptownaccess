...
export default function Section() {
  ...
  return <Screen><Stack.Screen options={{ title: sections.find(item => item.href === `/${section}`)?.title ?? 'Explore PTown' }} /><PageHeader eyebrow={content.eyebrow} title={content.title} description={content.description} /><PreviewNotice />{section === 'reservations' && <ReservationPlanner />}{section === 'memberships' && <MembershipPlanner />}{section === 'media' && <MediaLibrary />}{division ? <><SectionHeader title="Explore planned pathways" />{pathways.filter(pathway => pathway.division === division).map(pathway => <PathwayCard key={pathway.id} pathway={pathway} />)}<Button label="Browse all creative pathways" href="/creative" secondary /></> : content.items.map(item => <Card key={item.title} {...item} />)}{section === 'memberships' && <Button label="Explore VIP Society" href="/vip" secondary />}<Button label="Explore PTown" href="/ptown" secondary /><Footer /></Screen>;
}
