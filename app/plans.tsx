...
export default function Plans() {
  ...
  return <Screen>
    ...
    {!store.ready ? <><Body>{store.storageError ? 'Your plan cannot be reviewed until this device’s saved data is reset.' : 'Loading your saved plan…'}</Body><Button label="Open saved plans" href="/profile" secondary /></> : !summary.hasPlans ? <><Card title="Start with a good idea" description="Save a proposed program, a dinner draft, or a membership interest. Your review and shareable summary will appear here." /><Button label="Browse programs" href="/events" /><Button label="Plan a dinner draft" href="/reservations" secondary /></> : <>
      <View style={styles.card}><SectionHeader title="Saved ideas, ready to review" /><Body>{summary.programs.length} saved {summary.programs.length === 1 ? 'program' : 'programs'} · {summary.tournamentInterests.length} tournament {summary.tournamentInterests.length === 1 ? 'interest' : 'interests'} · {store.reservationDraft ? 'Dinner draft saved' : 'No dinner draft'} · {store.membershipInterest ? 'Membership interest saved' : 'No membership interest'}</Body><Button label="Edit your saved plans" href="/profile" secondary /></View>
      ...
      <SectionHeader title="Your tournament interests" />
      {summary.tournamentInterests.length ? <Card title={summary.tournamentInterests.map(game => game.title).join(' · ')} description="Interest only. You are not registered, no place is reserved, and no attendance or check-in has been recorded." /> : <Card title="No tournament interests saved" description="Choose games in the Tournament Hub if you want them included in this planning summary." />}
      <Button label="Review tournament interests" href="/tournament" secondary />
      ...
      {store.reservationDraft?.notes?.trim() && <Card title="Dinner planning note" description={`${store.reservationDraft.notes} · Planning only; PTown has not received or confirmed this request.`} />}
