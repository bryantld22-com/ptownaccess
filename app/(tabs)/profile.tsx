...
export default function Profile() {
  ...
  const hasPlans = savedEvents.length > 0 || reservationDraft !== null || membershipInterest !== null;
  ...
  return <Screen>
    <PageHeader eyebrow="YOUR PTOWN PLANS" title="Keep the good ideas." description="Saved programs, a dinner draft, and your membership interests—all in one place." />
    <PreviewNotice />
    ...
    {(hasPlans || storageError) && <View style={styles.card}>
      ...
      {confirmClear ? <><Body>Clear all saved events, your reservation draft, and membership interest from this device? This also resets unreadable preview data.</Body><View style={formStyles.row}><ActionButton label="Clear my saved plans" disabled={busy} onPress={() => { void clear(); }} /><ActionButton label="Keep my plans" disabled={busy} secondary onPress={() => setConfirmClear(false)} /></View></> : <><Body>You can remove this device’s preview plans at any time.</Body><ActionButton label="Clear saved preview data" disabled={busy} secondary onPress={() => { setMessage(null); setConfirmClear(true); }} /></>}
    </View>}
