...
export default function BookingCalendarScreen() {
  ...
  const { bookings, getArtist, saveBooking, removeBooking, busy, canEdit, currentRole } = useOperationsStore();
  ...
  const conflicts = validDate ? bookings.filter(booking => booking.date === date) : [];
  ...
  return <Screen>
    ...
    <View style={styles.access}><Text style={styles.accessText}>{currentRole} · {canEdit ? 'Calendar editing enabled' : 'Read-only calendar'}</Text><Button label="Access roles" href="/operations/access" secondary /></View>
    ...
    {conflicts.length > 0 && <View style={styles.conflict}><Text style={styles.conflictTitle}>DATE CONFLICT WARNING</Text><Text style={styles.conflictText}>{conflicts.length} existing {conflicts.length===1?'entry':'entries'} on {date}: {conflicts.map(item => `${getArtist(item.artistId)?.name ?? 'Artist'} (${item.status})`).join(', ')}. Review before adding another hold or offer.</Text></View>}
...
const styles=StyleSheet.create({access:{backgroundColor:theme.colors.elevated,borderRadius:14,padding:12,gap:10},accessText:{color:theme.colors.cream,fontSize:12,fontWeight:'700'},conflict:{backgroundColor:'#2E2018',borderWidth:1,borderColor:'#A96F45',borderRadius:14,padding:14,gap:5},conflictTitle:{color:'#E8B07E',fontSize:10,fontWeight:'900',letterSpacing:1.2},conflictText:{color:theme.colors.cream,fontSize:12,lineHeight:18},legend:{flexDirection:'row',flexWrap:'wrap',gap:12},legendItem:{flexDirection:'row',alignItems:'center',gap:6},dot:{width:8,height:8,borderRadius:4},legendText:{color:theme.colors.muted,fontSize:11},label:{color:theme.colors.cream,fontSize:15,fontWeight:'600'},choices:{flexDirection:'row',flexWrap:'wrap',gap:8},choice:{borderWidth:1,borderColor:theme.colors.border,borderRadius:20,paddingHorizontal:12,paddingVertical:9},choiceSelected:{backgroundColor:theme.colors.gold,borderColor:theme.colors.gold},choiceText:{color:theme.colors.cream,fontSize:12,fontWeight:'700'},choiceTextSelected:{color:theme.colors.background},entry:{backgroundColor:theme.colors.surface,borderWidth:1,borderColor:theme.colors.border,borderRadius:16,padding:14,flexDirection:'row',alignItems:'center',gap:12},dateBlock:{borderLeftWidth:3,paddingLeft:10,minWidth:92},date:{color:theme.colors.cream,fontSize:12,fontWeight:'800'},stage:{color:theme.colors.muted,fontSize:10,marginTop:4},warning:{color:'#E8B07E',fontSize:8,fontWeight:'900',marginTop:4},artist:{color:theme.colors.cream,fontSize:15,fontWeight:'800'},meta:{color:theme.colors.gold,fontSize:11},notes:{color:theme.colors.muted,fontSize:11},remove:{color:'#E8AAA2',fontSize:11,padding:8}});
