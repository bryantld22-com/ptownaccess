...
export default function ArtistCrmScreen() {
  ...
  const artists = artistLeads.map(artist => getArtist(artist.id) ?? artist);
  return <Screen>
    ...
    <Link href="/operations/calendar" style={styles.calendarLink}>Open booking calendar →</Link>
    <View style={styles.summary}>{(['Identified','Contacted','Warm','Negotiating','Booked'] as const).map(status => <View key={status} style={styles.stat}><Text style={styles.statValue}>{artists.filter(a => a.status === status).length}</Text><Text style={styles.statLabel}>{status}</Text></View>)}</View>
    {artists.map(artist => <Link key={artist.id} href={{ pathname: '/operations/artists/[id]', params: { id: artist.id } }} asChild><Pressable accessibilityRole="link" style={styles.card}>
...
const styles = StyleSheet.create({
  ...
  calendarLink:{color:theme.colors.gold,fontSize:14,fontWeight:'800',paddingVertical:8},
