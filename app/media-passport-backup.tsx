...
import { createMediaPassportTransfer, MAX_MEDIA_PASSPORT_TRANSFER, planMediaPassportMerge, readMediaPassportTransfer, type MediaPassportMergeChoice } from '../src/utils/mediaPassportTransfer';
...
export default function MediaPassportBackup() {
  ...
  const mergePlan = incoming ? planMediaPassportMerge(drafts, incoming, choices) : null;
  async function copy() {
  ...
  function review() {
    setMessage(null); setError(null); setIncoming(null); setChoices({});
  ...
  async function merge() {
    if (!incoming || !mergePlan || mergePlan.unresolved.length || mergePlan.overLimit || !ready || storageError) return;
    setBusy(true); setMessage(null); setError(null);
    try {
      const current = readMediaPassports(await AsyncStorage.getItem(MEDIA_PASSPORTS_KEY));
      if (JSON.stringify(current) !== JSON.stringify(drafts)) throw new Error('Current drafts changed');
      const plan = planMediaPassportMerge(current, incoming, choices);
      if (plan.unresolved.length || plan.overLimit) throw new Error('Merge not ready');
      ...
      setDrafts(validated); setIncoming(null); setChoices({}); setCodeInput('');
      setMessage(`Merged ${plan.added} new drafts and reviewed ${plan.conflicts.length} changed drafts on this device. No mentor was notified.`);
  ...
  return <Screen>
    <PageHeader eyebrow="PTOWN MEDIA ACADEMY · BUILD 87" title="Move Skills Passport drafts carefully." description="Copy a private transfer code, review its contents, then merge or replace saved drafts on another device." />
    <PreviewNotice />
    ...
    <Field label="Paste a Skills Passport transfer code" value={codeInput} onChangeText={value => { setCodeInput(value); setIncoming(null); setChoices({}); setMessage(null); setError(null); }} maxLength={MAX_MEDIA_PASSPORT_TRANSFER} multiline autoCapitalize="none" autoCorrect={false} style={{ minHeight: 180, textAlignVertical: 'top' }} />
    ...
    {incoming && mergePlan && <><Card title={`${incoming.length} incoming ${incoming.length === 1 ? 'draft' : 'drafts'}`} description={`Merge can add ${mergePlan.added} new drafts while keeping current records. ${mergePlan.identical} are identical. ${mergePlan.conflicts.length} changed records use the same ID and need a choice. Review the entries below.`} />{incoming.map(item => <Card key={item.id} title={`${item.participant} · ${item.divisionId}`} description={`Skills with evidence noted: ${item.skills.join(', ') || 'None'}\nPortfolio evidence: ${item.evidence}\nProposed mentor: ${item.mentor || 'To assign'}\nUpdated: ${item.updatedAt.slice(0, 10)}`} />)}
      ...
      {mergePlan.duplicates.length > 0 && <Card title={`${mergePlan.duplicates.length} possible duplicate entries`} description="Different IDs share a participant name and division. Merge keeps both; inspect their evidence before proceeding." />}
      ...
      <ActionButton label={`Merge reviewed drafts (${mergePlan.unresolved.length} choices remaining)`} disabled={busy || mergePlan.unresolved.length > 0 || mergePlan.overLimit} onPress={() => { void merge(); }} />
