...
import { createMediaPassportTransfer, MAX_MEDIA_PASSPORT_TRANSFER, readMediaPassportTransfer } from '../src/utils/mediaPassportTransfer';
...
export default function MediaPassportBackup() {
  ...
  function review() {
    setMessage(null); setError(null); setIncoming(null);
  ...
  return <Screen>
    <PageHeader eyebrow="PTOWN MEDIA ACADEMY · BUILD 84" title="Move Skills Passport drafts carefully." description="Copy a private transfer code, review its contents, and replace the saved passport list on another device." />
    <PreviewNotice />
    ...
    <Field label="Paste a Skills Passport transfer code" value={codeInput} onChangeText={value => { setCodeInput(value); setIncoming(null); setMessage(null); setError(null); }} maxLength={MAX_MEDIA_PASSPORT_TRANSFER} multiline autoCapitalize="none" autoCorrect={false} style={{ minHeight: 180, textAlignVertical: 'top' }} />
    <ActionButton label="Review incoming drafts" disabled={!ready || storageError || busy || !codeInput.trim()} secondary onPress={review} />
    {incoming && <><Card title={`${incoming.length} incoming ${incoming.length === 1 ? 'draft' : 'drafts'}`} description={`Replacing will remove all ${drafts.length} currently saved drafts on this device. This is a replacement, not a merge. Review the entries below before continuing.`} />{incoming.map(item => <Card key={item.id} title={`${item.participant} · ${item.divisionId}`} description={`Skills with evidence noted: ${item.skills.join(', ') || 'None'}\nPortfolio evidence: ${item.evidence}\nProposed mentor: ${item.mentor || 'To assign'}\nUpdated: ${item.updatedAt.slice(0, 10)}`} />)}<ActionButton label="Replace current Skills Passports with reviewed drafts" disabled={busy} onPress={() => { void replace(); }} /></>}
